...
inline double airViscosity(double T_kelvin) {
    constexpr double mu0 = 1.716e-5;
    constexpr double T0  = 273.15;
    constexpr double S   = 110.4;
    return mu0 * std::pow(T_kelvin / T0, 1.5) * (T0 + S) / (T_kelvin + S);
}


inline double airTemperature(double h_m) {
    struct Layer { double h, T, alpha; };
    constexpr Layer layers[] = {
        {     0.0, 288.15, -6.5e-3 },
        { 11000.0, 216.65,  0.0    },
        { 25000.0, 216.65, +3.0e-3 },
        { 47000.0, 282.65,  0.0    },
        { 53000.0, 282.65, -4.5e-3 },
        { 79000.0, 165.65,  0.0    },
    };

    const Layer* layer = &layers[0];
    for (int i = 1; i < 6; ++i) {
        if (h_m < layers[i].h) break;
        layer = &layers[i];
    }
    return layer->T + layer->alpha * (h_m - layer->h);
}

class GOST4401_81Raw {
public:
    constexpr static int N = 475;

    constexpr static std::array<double, N> height = {
        ...
        256000,  257000,  258000,  259000,  260000,  261000,  262000,  263000,  264000,    266000,  267000,
    ...
    constexpr static std::array<double, N> density = {
        ...
        5.94575e-08, 5.17245e-08, 4.51814e-08, 3.96418e-08, 3.49222e-08, 3.08814e-08, 2.74061e-08,
        ...
        4.30201e-11, 4.20071e-11, 4.00635e-11, 3.91313e-11, 3.82245e-11, 3.73422e-11, 3.64837e-11,
    ...
    constexpr static double MIN_HEIGHT = height[0];
    constexpr static double MAX_HEIGHT = height[N - 1];

    static double getDensity(double h) {
        if (h < MIN_HEIGHT) {
            constexpr double g0M_R = 0.034164;

            struct Layer { double h, T, alpha, p; };
            constexpr Layer layers[] = {
                {     0.0, 288.15, -6.5e-3, 101325.0   },
                { 11000.0, 216.65,  0.0,     22632.1    },
                { 25000.0, 216.65, +3.0e-3,   2549.22   },
                { 47000.0, 282.65,  0.0,       110.906  },
                { 53000.0, 282.65, -4.5e-3,    66.9389  },
                { 79000.0, 165.65,  0.0,         1.0219 },
            };

            const Layer* L = &layers[0];
            for (int i = 1; i < 6; ++i) {
                if (h < layers[i].h) break;
                L = &layers[i];
            }

            double T = L->T + L->alpha * (h - L->h);
            double p;
            if (std::abs(L->alpha) < 1e-12) {
                p = L->p * std::exp(-g0M_R / L->T * (h - L->h));
            } else {
                p = L->p * std::pow(T / L->T, -g0M_R / L->alpha);
            }

            constexpr double R_spec = 287.05; // Дж/(кг·К)
            return p / (R_spec * T);
        }
        ...
        double t = (h - height[lo]) / (height[hi] - height[lo]);
...
public:
    static double get(double Re) {
        ...
        if (Re > 5e5)
            return 0.2;
        return interpolate(Re);
    }

private:
    constexpr static int N = 36;

    constexpr static std::array<double, N> re_table = {
        1.0,   2.0,   3.0,   5.0,   7.0,
        10.0,  20.0,  30.0,  50.0,  70.0,
        100.0, 200.0, 300.0, 500.0, 700.0,
        1.0e3, 2.0e3, 3.0e3, 5.0e3, 7.0e3,
        1.0e4, 2.0e4, 3.0e4, 5.0e4, 7.0e4,
        1.0e5, 1.5e5, 2.0e5, 2.5e5, 3.0e5,
        3.5e5, 4.0e5, 4.5e5, 5.0e5, 5.5e5,
        6.0e5
    };

    constexpr static std::array<double, N> cd_table = {
        24.0,  13.0,  9.0,   6.0,   4.6,
        4.1,   2.6,   2.0,   1.5,   1.2,
        1.07,  0.77,  0.65,  0.55,  0.50,
        0.46,  0.42,  0.42,  0.43,  0.39,
        0.41,  0.45,  0.46,  0.47,  0.47,
        0.48,  0.49,  0.50,  0.50,  0.46,
        0.36,  0.25,  0.20,  0.20,  0.20,
        0.20
    };

    static double interpolate(double Re) {
        const double logRe = std::log10(Re);
        int lo = 0, hi = N - 1;
        while (hi - lo > 1) {
            int mid = (lo + hi) / 2;
            if (re_table[mid] <= Re) lo = mid;
            else                     hi = mid;
        }
        const double t = (logRe - std::log10(re_table[lo]))
                       / (std::log10(re_table[hi]) - std::log10(re_table[lo]));
        const double logCd = std::log10(cd_table[lo])
                           + t * (std::log10(cd_table[hi]) - std::log10(cd_table[lo]));
        return std::pow(10.0, logCd);
    }
};


inline double cylinderDragForcePerLength(double v, double r, double h) {
    const double rho = GOST4401_81Raw::getDensity(h);

    double Cd;
    if (h <= 80000.0) {
        const double T  = airTemperature(h);
        const double mu = airViscosity(T);
        const double Re = rho * v * (2.0 * r) / mu;
        Cd = DragCoefficient::get(Re);
    } else {
        Cd = 2.2;
    }

    return Cd * 0.5 * rho * v * v * (2.0 * r);
}
